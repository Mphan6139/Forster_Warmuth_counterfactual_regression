...
single_gen_QIV = function(n,nc=3){
...
my_SL_QIV = function(data_train, data_pred, choice = c(1,7)){
  ...
  model_a_z = MySL(Data = data_train,              # training dataset
                 ...
                 locX = c(1,2,4),                  # explanatory variable X1,X2
  ...
  model_y   =  MySL(Data = data_train,             # training dataset
                 ...
                 MLPdecay = SL.hpara$MLPdecay)
  # for delta_a
  pi_1 = predict(model_a_z, data.frame(data_pred[,1:2],z=1) , onlySL = TRUE)[[1]]
  pi_0 = predict(model_a_z, data.frame(data_pred[,1:2],z=0) , onlySL = TRUE)[[1]]
  
  # for psi
  Ey_z1 = predict(model_y_z, data.frame(data_pred[,1:2],z=1), onlySL = TRUE)[[1]]
  Ey_z0 = predict(model_y_z, data.frame(data_pred[,1:2],z=0), onlySL = TRUE)[[1]]
  
  #for phi_1
  Ey_a1z1 = predict(model_y_za, data.frame(data_pred[,1:2],z=1,a=1), onlySL = TRUE)[[1]]
  phi_0 = predict(model_y_za, data.frame(data_pred[,1:2],z=0,a=1), onlySL = TRUE)[[1]]
  
  #for p_Y_0
  p_Y = predict(model_y, data.frame(data_pred[,1:2]), onlySL = TRUE)[[1]]
  ...
  a = data_pred[[5]]
  p_z = z*0.5 + (1-z)*0.5
  # P(A|Z,X) = ZP(A|Z=1,X) +(1-Z)P(A|Z=0|X) 
  p_az = p_z*(z*pi_1 + (1-z)*pi_0)
  
  phi_1 = Ey_a1z1-phi_0
  psi = (Ey_z1 - Ey_z0-phi_1)/(pi_1-pi_0)
  p_Y_0 = p_Y - data_pred$a*psi - data_pred$z*phi_1
  df_ = data.frame(pi_1,
              pi_0,
              phi_1,
              phi_0,
...
construct_pseudo_QIV = function(y,a,z,p_a,pi_1,pi_0,p_z,p_az,phi_1,phi_0,psi,p_Y_0, EIF=FALSE){
  #wald: E(psi|A=1)
  # y = Outcome
  # z = IV
  # a = Treatment
  ...
  # Check math but hope this works?
  
  p_a_x = (z*pi_1 + (1-z)*pi_0)
  
  nu_1 = p_a_x/p_a*(2*z-1)/p_z/delta_a
  nu_2 = y - a*psi - z*phi_1 - p_Y_0
  nu_3 = p_a_x/p_a*a*(2*z-1)/p_az/delta_a
...
my_fw_QIV = function(data_train, data_pred, bs = "bs", k = 4, choice = c(1,7), CV = F, df_grid = NULL){
  ...
  df_nuisance_1 = my_fw_aux_QIV(data_train = data_train[s==1,], aux = data_train[s==2,],
                            data_pred = data_pred, CV = CV, k = k, bs=bs, choice = choice)
  df_nuisance_2 = my_fw_aux_QIV(data_train = data_train[s==2,], aux = data_train[s==1,],
                            data_pred = data_pred, CV = CV, k = k, bs=bs, choice = choice)
...
my_fw_aux_QIV = function(data_train, aux, data_pred, bs = "bs", k = 4, choice = c(1,7), CV = F, df_grid = NULL){
  ...
  n_train = dim(data_train_)[1]
  n_aux = dim(aux_)[1]
  n_pred = dim(data_pred_)[1]
  
  # use nuisance_df1 to construct the pseudo outcome
  SL_res = my_SL_QIV(data_train_, rbind(aux_,data_pred_), choice = choice)
  df_aux = SL_res[1:n_aux,]
  df_pred = SL_res[ (n_aux+1):(n_aux+n_pred), ]


  pseudo_wald_if = construct_pseudo_QIV(y = aux_$y,
                                        a = aux_$a,
                                        z = aux_$z,
                                        pi_1 = df_aux$pi_1,
                                        pi_0 = df_aux$pi_0,
  ...
  if_qiv = construct_pseudo_QIV(y = aux_$y,
                                a = aux_$a,
                                z = aux_$z,
                                pi_1 = df_aux$pi_1,
                                pi_0 = df_aux$pi_0,
  ...
  nuisance_df = data.frame(y,a,z,pi_1,pi_0,p_z,p_az,phi_1,phi_0,psi,p_Y_0,if_qiv,pseudo_wald_if,att_pred)
  return(nuisance_df)
}
...
sim_SL = my_SL_QIV(data_train_,data_pred_)


# NOT cross-validated
sim_1 = my_fw_aux_QIV(data_train_,aux_,data_pred_)
# Wald-estimand based estimator 
mean(sim_1$psi)
# EIF based on proof
mean(sim_1$if_qiv)
# FW based on Psuedo-outcome from Wald Efficient Influence Function
mean(sim_1$pseudo_wald_if)
mean(sim_1$att_pred)
...
three_fold_eval = function(data_input, CV = F, k=4, bs = 'bs', choice = c(1,7)){
  ...
  get_one = function(combination){
    ...
    nuisance_df_two_step = my_fw(data_train = Data[s==train_1 | s==train_2,],
                                 data_pred = Data[s==pred,], CV = CV, k = k, 
                                 bs=bs, choice =choice)
    
    estimate_wald = with(nuisance_df_one_step,(psi * a[s==pred]  / mean(a[s==pred])))
    estimate_FW = with(nuisance_df_two_step, )
...
single_run_simulation = function(N,nc=3, fold_num = 1, CV,seed = NULL, k=4, bs= 'bs',choice=c(1,5,7), raw = F){
  ...
  generated = single_gen(N)
  current_df = generated[1]
  att = generated[2]
...
#three_fold_eval(data_input, CV=T, k=3, bs = 'bs', choice=c(1,2,3,4,5,6,7))
# r1 = single_run_simulation(N=300,CV=T,k=3,choice=c(1,2,3,4,5,6,7))
