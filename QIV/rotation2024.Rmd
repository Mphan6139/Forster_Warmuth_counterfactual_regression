...
my_SL_QIV = function(data_train, data_pred, choice = c(1,7)){
  ...
  phi_1 = Ey_a1z1-phi_0
  #marginalize out A in Y|Z,A. 
  Ey_z1 = Ey_a1z1*pa_x + predict(model_y_za, data.frame(X,z=1,a=0))[[1]]*(1-pa_x)
  Ey_z0 = phi_0*pa_x + predict(model_y_za, data.frame(X,z=0,a=0))[[1]]*(1-pa_x) 
  psi = (Ey_z1 - Ey_z0-phi_1)/(pi_1-pi_0)

  fz_x = z*pz_x + (1-z)*(1-pz_x)
  pa_zx = predict(model_a_z, data.frame(data_pred[,c(1:2,4)]))[[1]]
  paz_x = a*pa_zx*fz_x + (1-a)*(1-pa_zx)*fz_x 
  
  #marginalize out z
  y_L = Ey_z1*pz_x + Ey_z0*(1-pz_x)
  
  p_Y_0 = y_L - psi*pa_x - phi_1*pz_x
...
three_fold_eval = function(data_input, CV = F, k=4, bs = 'bs', choice = c(1,7)){
  ...
  get_one = function(combination){
    ...
    #                              bs=bs, choice = choice)
    # 
    # This is a marginal estimate (scalar)
    estimate_wald = mean(nuisance_df_one_step$psi*nuisance_df_one_step$a/mean(nuisance_df_one_step$a))
    
    EIF_est = construct_psi_hat(y = nuisance_df_one_step$y,
                                a = nuisance_df_one_step$a,
                                z = nuisance_df_one_step$z,
                                pi_1 = nuisance_df_one_step$pi_1,
                                pi_0 = nuisance_df_one_step$pi_0,
                                pz_x = nuisance_df_one_step$pz_x,
                                pa_x = nuisance_df_one_step$pa_x,
                                paz_x = nuisance_df_one_step$paz_x,
                                phi_1 = nuisance_df_one_step$phi_1,
                                phi_0 = nuisance_df_one_step$phi_0,
                                psi = nuisance_df_one_step$psi,
                                p_Y_0 = nuisance_df_one_step$p_Y_0)
    

    estimate_EIF = EIF_est[[1]]
    var_EIF = EIF_est[[2]]
...
single_run_simulation = function(N,nc=3, fold_num = 1, CV,seed = NULL, k=4, bs= 'bs',choice=c(1,5,7), raw = F){
  ...
  bias_single = apply(do.call(rbind, bias_list), 2, median, na.rm = TRUE) 
...
simple_large_sim = function(N, fold_num = 1){
  generated = single_gen_QIV(N*fold_num)
  s <- rep(1:2,(N*fold_num)/2)
  
  data_input = generated[[1]]
  att = generated[[2]]
  data_input$s = s
  
  wald_list = rep(0, fold_num)
  est_list = rep(0, fold_num)
  var_list = rep(0, fold_num)
  
  
  for(j in 1:fold_num){
    data_input_j = data_input[(N*(j-1)+1):(N*j),]
    nuisance_df = my_SL_QIV(data_train = data_input_j[data_input_j$s==1,],
                            data_pred = data_input_j[data_input_j$s==2,])
    estimate_wald = mean(nuisance_df$psi*nuisance_df$a/mean(nuisance_df$a))
    
    EIF_est = construct_psi_hat(y = nuisance_df$y,
                                a = nuisance_df$a,
                                z = nuisance_df$z,
                                pi_1 = nuisance_df$pi_1,
                                pi_0 = nuisance_df$pi_0,
    ...
    wald_list[j] = estimate_wald
    est_list[j] = estimate_EIF
    var_list[j] = var_EIF
  }
  MC_variance = sum((est_list- mean(est_list))^2)/fold_num
  A_variance = var_list
  CI = list(est_list - 1.96*sqrt(var_EIF),est_list + 1.96*sqrt(var_EIF))  
  coverage = sum((CI[[1]] < att)*(att < CI[[2]]))/fold_num
  return(list(bias_wald = mean(wald_list-att),
              bias_eif = mean(est_list-att),
              med_bias = median(est_list-att),
...
sim_1 = simple_large_sim(N=100,fold_num = 500)

sim_2 = simple_large_sim(N=500,fold_num = 500)

sim_3 = simple_large_sim(N=1000,fold_num = 500)

sim_4 = simple_large_sim(N=5000,fold_num = 500)

sims_res = list(sim_1,sim_2,sim_3,sim_4)

save(sims_res, file = "./sims")
...
options(warn=-1)
r1 = single_run_simulation(N=2400,nc=3,CV=T,k=3,choice=c(1,2,3,4,5,6,7))
options(warn=0)
...
```{r}
# TRASH CODE
# construct_pseudo_QIV = function(y,a,z,pi_1,pi_0,pz_x,pa_x,paz_x,phi_1,phi_0,psi,p_Y_0, EIF=FALSE){
#   #wald: E(psi|A=1)
#   # y = Outcome
#   # z = IV
#   # a = Treatment
...
#   
#   
#   return(nu_1*nu_2 - nu_3*nu_4 + nu_6)
# }
```
