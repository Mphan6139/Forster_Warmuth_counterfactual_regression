...
my_SL_QIV = function(data_train, data_pred, choice = c(1,7)){
  ...
  # model this later
  p_z = z*0.5 + (1-z)*0.5
  # P(A,Z|X) = P(Z=1)P(A|Z=1,X) 
  p_az = pi_1*p_z + pi_0*(1-p_z)
...
construct_psi_hat = function(y,a,z,p_a,pi_1,pi_0,p_z,p_ax,p_az,phi_1,phi_0,psi,p_Y_0){
  #wald: E(psi|A=1)
  # y = Outcome
  # z = IV
  # a = Treatment
...
three_fold_eval = function(data_input, CV = F, k=4, bs = 'bs', choice = c(1,7)){
  ...
  get_one = function(combination){
    ...
    EIF_est = construct_psi_hat(y = nuisance_df_one_step$y,
                                ...
                                p_z = nuisance_df_one_step$p_z,
                                p_ax = nuisance_df_one_step$p_ax,
                                p_az = nuisance_df_one_step$p_az,
...
simple_large_sim = function(N, fold_num = 1){
  ...
  for(j in 1:fold_num){
    ...
    EIF_est = construct_psi_hat(y = nuisance_df$y,
                                ...
                                p_z = nuisance_df$p_z,
                                p_ax = nuisance_df$p_ax,
                                p_az = nuisance_df$p_az,
  ...
  A_variance = 0
  CI = c(est_list - 1.96*sqrt(var_EIF),est_list + 1.96*sqrt(var_EIF))  
  covarage = sum((CI[1] < att)*(att < CI[2]))/fold_num
  return(list(bias= mean(est_list-att),
...
simple_large_sim(N=100,fold_num = 10)
