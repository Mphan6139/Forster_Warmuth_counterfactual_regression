...
colnames(var_mat) = colnames(cate_hat_mat) =colnames(res)= c("plugin","forster_poly","forster_ns","forster_bs", "forster_poly_cross","forster_ns_cross","forster_bs_cross")
var_mat$N = n_vec
res$N = n_vec
...
cross_fit <- function(df_, I_1_, I_2_,type_,basis_){
  forster_1 = series_df(I_1_$X,I_1_$PsuedoO,df_$X,df=4, type = type_, basis_type = basis_)[[1]]
  
  forster_2 = series_df(I_2_$X,I_2_$PsuedoO,df_$X,df=4, type = type_, basis_type = basis_)[[1]]
  
  return(1/2*(forster_1+forster_2))
  
}
  
  forster_poly_cross = cross_fit(df, I_1, I_2, "forster" , "poly" )
  forster_ns_cross = cross_fit(df, I_1, I_2, "forster" , "ns" )
  forster_bs_cross = cross_fit(df, I_1, I_2, "forster" , "bs" )
  ...
  lm(y ~ poly(x, degree = 3, raw = TRUE, data = summary(smooth.spline(df$X, df$Y))))
...
```{r}
# Variance plots
FW_fit_1 = series_df(I_1$X,I_1$PsuedoO,I_2$X,df=4, type = "forster", basis_type = "bs")[[1]]
FW_fit_2 = series_df(I_2$X,I_2$PsuedoO,I_1$X,df=4, type = "forster", basis_type = "bs")[[1]]

cross_fit_CATE = c(FW_fit_1,FW_fit_2)
df$f_hat = cross_fit_CATE

EY1hat <- predict(smooth.spline(I_1$X[I_1$Z==1] , I_1$Y[I_1$Z==1]),I_1$X)$y
EY0hat <- predict(smooth.spline(I_1$X[I_1$Z==0] , I_1$Y[I_1$Z==0]),I_1$X)$y
...
ggplot(df, aes(X))+
  geom_line(aes(y = f_hat),colour="blue") 
