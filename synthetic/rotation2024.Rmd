...
for (i in 1:nsim){
  ...
  error <- rnorm(n,sd=1)
  p_2 <- function(x,a,u){
    f_x = (x <= -.5)*0.5*(x+2)^2 + (x/2+0.875)*(x>-1/2 & x<0) +
  ...
  po_fitter <- function(sub_df){
    df_z1 = sub_df[sub_df$Z==1,]
    df_z0 = sub_df[sub_df$Z==0,]
    ## estimate nuisance functions
    # want to emphasize ability to use non-parametric nuissance functions  

  
    model_A1 = glm(A ~ ns(X),data = df_z1,family = binomial())
    model_A0 = glm(A ~ ns(X),data = df_z0,family = binomial())
