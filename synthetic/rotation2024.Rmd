...
for (i in 1:nsim){
  ...
  ps <- 0.2*expit(2*x) + 0.4*z + 0.1*(u>0)
  range(ps)
  a <- rbinom(n,1,ps);
...
ggplot(res, aes(x = N))+
  ...
  geom_line(aes(y = forster_bs_cross, colour = "forster_bs_cross"), linewidth = s_)+
  
  # geom_line(aes(y = ls_ns, colour = "ls_ns"), linewidth = s_)+
  geom_line(aes(y = forster_ns_cross, colour = "forster_ns_cross"), linewidth = s_)+
  
  # geom_line(aes(y = ls_poly, colour = "ls_poly"), linewidth = s_)+
  geom_line(aes(y = forster_poly_cross, colour = "forster_poly_cross"), linewidth = s_)+
...
ggplot(temp_df) + 
  geom_line(aes(x=x_new,y=forster_bs_cross,color = "bs"), linewidth = s_) + 
  geom_line(aes(x=x_new,y=forster_ns_cross,color = "ns"), linewidth = s_) +
  geom_line(aes(x=x_new,cate,color = "CATE(X)"), linewidth = s_) +
  geom_line(aes(x=x_new,plugin,color="plugin"), linewidth = s_) +
  geom_line(aes(x=x_new,y=forster_poly_cross,color="poly"), linewidth = s_) +

  ggtitle("CATE Function Estimates")+
  xlab("X")+
  ylab("CATE(X)")+
  scale_color_manual(values=c("#66A61E","black","#1B9E77","#E7298A","#E6AB02"))
