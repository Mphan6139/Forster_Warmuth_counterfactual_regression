...
for (i in 1:nsim){
  ...
  ps <- expit(4+2*x)*(0.2 + 0.6*z) + 0.1*(2*u-1)
  range(ps)
  ...
  # df$EA1hat <- predict(m_A1,newdata = df,type = "response")
  # df$EA0hat <- predict(m_A0,newdata = df,type = "response")
  plugin_f <- function(x){
    EA1hat_x <- expit(2+2*x)*(0.8)
    EA0hat_x <- expit(2+2*x)*(0.2)
    EY1hat_x <- predict(smooth.spline(df$X[df$Z==1] , df$Y[df$Z==1]),x)$y
    EY0hat_x <- predict(smooth.spline(df$X[df$Z==0] , df$Y[df$Z==0]),x)$y
    return (EY1hat_x-EY0hat_x)/(EA1hat_x - EA0hat_x)
  }
  
  
  plugin <- plugin_f(df$X)
  ...
  po_fitter <- function(sub_df){
    ...
    # use true model
    EA1hat = expit(2+2*sub_df$X)*(0.8) 
    EA0hat = expit(2+2*sub_df$X)*(0.2) 
    ...
    #UPDATE: We are multiply robust, look at the linbo paper
...
# Variance plots
DegF = 50
FW_fit_1 = series_df(I_1$X,I_1$PsuedoO,I_2$X,df=DegF, type = "forster", basis_type = "bs")[[1]]
FW_fit_2 = series_df(I_2$X,I_2$PsuedoO,I_1$X,df=DegF, type = "forster", basis_type = "bs")[[1]]
...
est_forster_poly=est_forster_bs = x1_list = seq(from=-1,to=1,by=0.1)
...
ggplot(temp_df) + 
  ...
  geom_line(aes(x=x1_list,plugin_f(x1_list),color="plugin"), size = s_) +
...
mse_bs = mean((est_forster_bs-cate_f(x1_list))**2)
mse_poly = mean((est_forster_poly-cate_f(x1_list))**2)
mse_plugin = mean((plugin_f(x1_list)-cate_f(x1_list))**2)
c(mse_bs,mse_poly,mse_plugin)
```
