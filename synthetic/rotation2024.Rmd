...
for (rate in rateseq){
  ...
  for (i in 1:nsim){
    ...
    u <- rnorm(n,sd=(.2-.1*cos(2*pi)))
    # IV
    z <- rbinom(n,1,prob = 0.8)
    ps <- 0.1 + 0.5*(x>0) + 0.2*z + 0.1*(u>0)
    a <- rbinom(n,1,ps);
    # alt: positivity assumption
    # ps <- (0.1 + 0.7*(x>0) + 0.2*(u>0))
    # a <- rbinom(n,1,ps)*z 
    ...
    pi1hat <- predict(glm(a[z==1,s==2] ~ x[z==1,s==2]),x)$y
    pi0hat <- predict(glm(a[z==0,s==2] ~ x[z==0,s==2]),x)$y
    mu1hat <- predict(smooth.spline(x[z==1 & s==2],y[z==1 & s==2]),x)$y
    mu0hat <- predict(smooth.spline(x[z==0 & s==2],y[z==0 & s==2]),x)$y
    
    betahat <- (mu1hat-mu0hat)/(pi1hat - pi0hat)
    deltahat <- (pi1hat - pi0hat)
    
    # no idea what f is 
    pseudoIV <- (2*z - 1)/f(x) * (y - a*betahat - mu0hat + pi0hat*betahat)/deltahat + betahat
