...
load(file = "pseudo_unconfound.RData")
pseudo_sl = all[[1]]
...
for (x1 in x1_list){
  ...
  tau_forster_bs = series_cv_new(X1,pseudo_sl,x1,  type = "forster", basis_type = "bs", KK=5)
  ...
  tau_forster_poly = series_cv_new(X1,pseudo_sl,x1,  type = "forster", basis_type = "poly", KK=5)
...
unconfoundedness_data = list(est_forster_poly,sd_forster_poly,est_forster_bs,sd_forster_bs,x1_list)
save(unconfoundedness_data, file = "unconfound_plotdata.RData")
